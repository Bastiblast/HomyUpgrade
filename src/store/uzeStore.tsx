...
const PDPurl =
	'https://share.amazon.com/sites/MRS1-PDP/Documents%20partages/MRS1-PDP/repartMRS1.html';
...
export const uzeStore = create<Store>((set, get) => ({
			...
			case 'production':
				GM_xmlhttpRequest({
					...
					onload: async function (response) {
						const jsonStart =
							response.responseText.indexOf('var activite = {');
						const jsonEnd = response.responseText.indexOf(
							'}',
							jsonStart,
						);
						const jsonString = response.responseText.substring(
							jsonStart + 15,
							jsonEnd + 1,
						);
						const json = JSON.parse(jsonString);
						const packSinglePDP = Object.values(json).filter(
							(row) => {
								const processName = row[2] as string;
								return (
									processName === 'Pack Single Medium' ||
									processName === 'Pack&Ship'
								);
							},
						);
						...
						set({
							PDPdata: packSinglePDP,
							totalHeadCount: packSinglePDP.length,
						});
					},
				});
				break;
	...
	getRodeoCapa: async () => {
		...
		const processPath = ['PickingNotYetPicked', 'PickingPicked'].map(
			(pp) => 'WorkPool=' + pp + '&',
		);
		...
		pickingArray.forEach((shipment) => {
			...
			if (mappingCPTpicking.has(cpt)) {
				mappingCPTpicking
					...
					.set(
						'picking',
						quantity + mappingCPTpicking.get(cpt).get('picking'),
					);
			} else {
		...
		transitArray &&
			transitArray.forEach((element) => {
				...
				const existingQuantity = inTransitTote.get(tote)
					? inTransitTote.get(tote)
					: 0;
				inTransitTote.set(tote, existingQuantity + quantiy);
			});

		prioTote.forEach(
			(tote) => inTransitTote.get(tote) && prioTote.delete(tote),
		);
		...
		pickedArray.forEach((shipment) => {
			...
			if (
				!mappingCPTpicked.has(cpt) &&
				mergeToteDetail.find((bt) => bt.tote === tote)
			) {
				mergeToteDetail.find((bt) => bt.tote === tote) &&
					mappingCPTpicked.set(
						cpt,
						new Map().set(
							tote,
							mergeToteDetail.find((bt) => bt.tote === tote)
		...
		CPTpickingArray.forEach((CPT) => {
			...
			if (mergedPickingAndPickedArray.get(cpt)) {
				mergedPickingAndPickedArray.set(
					cpt,
					new Map([
						...mergedPickingAndPickedArray.get(cpt),
						...listValue,
					]),
				);
			} else {
	...
	buildJSON: (csvRow) => {
		...
		const csvGroupByDZ = Object.groupBy(
			csvRow,
			(row) => row['Outer Scannable ID'],
		);
		Object.entries(csvGroupByDZ).forEach((entries) => {
			...
			Object.entries(csvGroupByDZ[key]).forEach((entries) => {
				...
				const zubGroup = Object.groupBy(
					value,
					(row) => row['Expected Ship Date'],
				);

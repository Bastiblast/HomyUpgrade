...
export const uzePackMan = create<uzePackMan>((set, get) => ({
	...
	getPackManData: () => {
		...
		const bodyTester =
			'{"warehouseId":"MRS1","startTime":1733217726.292,"endTime":1733221326.292}';
	...
	buildJSON: (csvRow) => {
		{
			...
			Object.entries(csvGroupByDZ).forEach((entries) => {
				...
				Object.entries(csvGroupByDZ[key]).forEach((entries) => {
					...
					const zubReduce = Object.entries(
						csvGroupByDZ[key][keyz],
					).forEach((entries) => {
						const [keyzz, values] = entries;
						csvGroupByDZ[key][keyz][keyzz] = values.reduce(
							(acc, val) => {
			...
			console.log(
				...
				csvRow.filter(
					(row) =>
						row['Outer Scannable ID'] === 'dz-P-OB-Single-cvg-111',
				),
			);

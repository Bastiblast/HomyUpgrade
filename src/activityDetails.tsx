...
export default function ActivityDetails() {
  ...
  return (
    <form className="grid grid-cols-2 grid-flow-row bg-white px-2 border-4 border-violet-100 h-full">
      <h2 className="col-span-2 font-bold text-xl">Activity details</h2>
      <span className="col-span-2 text-xs">Information pour les calcules de capacité et de priorisation des bacs.</span>
      <span className={`flex items-center justify-end pr-3`}>headcount</span>
      <input
        ...
        type="number"
        className={`input input-xs m-1 my-auto border-blue-400 ` + emptyInputColor[totalHeadCount ?? 0]}
      />
      ...
      <input
        ...
        className={'input input-xs m-1 my-auto border-blue-400 ' + emptyInputColor[UPH]}
      />

      <div className="flex justify-end items-center pr-3 text-end">Temps avant CPT</div>
      <input
        value={TBCPT}
        ...
        type="number"
        className={`input input-xs m-1 my-auto border-blue-400 ` + emptyInputColor[TBCPT]}
      />
    </form>
  )
}

...
export default function Buffer({ dz }) {
	...
	const prioToteNumber =
		CPTlist.length > 0
			? Object.entries(data[dz]).filter((filter) => {
					...
					CPTlist.forEach((cpt) => {
						!isPrio
							? (isPrio = JSON.stringify(data[dz][tote]).includes(
									cpt,
								))
							: null;
					});
	...
	const render = (prioToteNumber) => {
		...
		const bufferName =
			dz === 'dz-P-OB-Single-CartRunMove' ? 'CartRunMove' : 'Buffer';
		return (
			<>
				<span
					className={
						'mx-2 rounded-md bg-blue-500 px-2 font-bold transition-all hover:scale-[2] hover:font-bold'
					}
				>
				...
				{isPrio && (
					<span
						className={
							'mx-2 rounded-md bg-red-500 px-2 font-bold transition-all hover:scale-[2] hover:font-bold'
						}
					>

...
export default function CapaTable(data) {
  ...
  const noPackerWarning = totalHeadCount === 0 && (
    <span className="col-span-full">Indiquer un nombre de pack pour calculer la capacité.</span>
  )
  ...
  const noDataWarning = dataObject.size === 0 && (
    <span className="col-span-full">Il n'y a pas de CPT dans les 4 prochaines heures.</span>
  )
  ...
  const capaTable = (UPH, TBCPT, totalHeadCount) =>
    mapToArray.map((CPT, index) => {
      ...
      return (
        <>
          <div key={index} className={'flex items-center justify-center border-b-2 border-r-2 ' + colorRisk}>
            {header}
          </div>
          <div key={index + 1} className={'flex items-center justify-center border-b-2 border-r-2 ' + colorRisk}>
            {allBtNumber}
          </div>
          <div key={index + 2} className={'flex items-center justify-center border-b-2 border-r-2 ' + colorRisk}>
            {index === 0 ? unitNumber : allUnitNumber}
          </div>
          <div key={index + 3} className={'flex items-center justify-center border-b-2 border-r-2 ' + colorRisk}>
            {deadLineTime}min
          </div>
          <div key={index + 4} className={'flex items-center justify-center border-b-2 ' + colorRisk}>
            {packerNeeded}
          </div>
        </>
      )
  ...
  return (
    <div className="flex h-full w-full flex-row items-center justify-center bg-violet-100">
      <div className="grid grid-flow-row grid-cols-5 border-4 border-violet-100 bg-white p-2">
        <div className="h-8 border-b-2 border-r-2 px-4 text-center font-bold">CPT</div>

        <div className="h-8 border-b-2 border-r-2 px-4 text-center font-bold"> BT Prio</div>

        <div className="h-8 border-b-2 border-r-2 px-4 text-center font-bold">Units Prio</div>

        <div className="h-8 border-b-2 border-r-2 px-4 text-center font-bold">Remain</div>
        <div className="h-8 border-b-2 px-4 text-center font-bold">Attendu</div>
